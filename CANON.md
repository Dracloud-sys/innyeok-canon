...
캐논 문서는 파일당 20~110KB다. **통독하지 말 것.**

1. 이 `CANON.md`로 어떤 문서가 필요한지 정한다.
2. 그 문서의 **맨 위 `CANON-INDEX` 헤더**를 읽는다 (`head -40 <파일>`).
   한 줄 요약과 섹션별 행 번호 목차가 들어 있다.
3. 목차에서 필요한 구간만 `sed -n 'A,Bp' <파일>` 로 읽는다.
4. 문서 전체 읽기는 마지막 수단이다.

`CANON-INDEX` 헤더는 `world/`, `world/economy/`, `characters/`의 모든 문서에 있다.
**원문은 헤더 아래부터 그대로다** — 헤더는 주석 블록이며 내용을 바꾸지 않는다.
문서를 편집한 뒤에는 행 번호가 어긋나므로 생성기를 다시 돌려야 한다
(생성기와 요약 원문: `Dracloud-sys/claude_study` → `scripts/canon_index.py`).
...
> ### ⚠️ 비공개 캐논 (프로덕션 취급 주의)
>
> **[overview.md 9-7항 — 영원의 실제 구조](world/overview.md)** 는 인역 사회에
> 알려져 있지 않은 진실이다. 캐논은 이를 사실로 확정하되, **프로덕션에서 인물이
> 이를 아는 것으로 서술해서는 안 된다.** 인물이 도달 가능한 최대치는 정황 종합에
> 의한 결론이며 직접 확인은 불가하다. 학계 통설은 9-6항(영기 맥 집중점)이다.
>
