# 인역(人域) 캐논 인덱스

> 세계관의 단일 진실 공급원(SSOT). 모든 프로덕션(소설·게임·영상/음악·현실무공)은 이 문서들을 참조만 하고 소유하지 않는다.
> 운영 규칙은 [ops/인역_IP_운영매뉴얼_v1.0.md](ops/인역_IP_운영매뉴얼_v1.0.md) 참조.

## 세계관 개요

| 문서 | 내용 | 상태 |
|------|------|:---:|
| [world/overview.md](world/overview.md) | 세계관 종합 개요 (구 innyeok-v4) | ✅ |
| [world/overview-archive/](world/overview-archive/) | 구버전 개요 (v2, v3) — 참고용, 캐논 아님 | 보관 |

> ### ⚠️ 비공개 캐논 (프로덕션 취급 주의)
>
> **[overview.md 9-7항 — 영원의 태고 인공 구조](world/overview.md)** 는 인역 사회에
> 알려져 있지 않은 진실이다. 캐논은 그 기원과 안정화 구조를 사실로 확정하되,
> **프로덕션에서 인물이 이를 아는 것으로 서술해서는 안 된다.** 인물이 도달 가능한
> 최대치는 정황 종합에 의한 결론이며 직접 확인은 불가하다. 영원의 공급원이
> 인역 내부 재배치인지 경계간 교환인지는 미규명이다.
>
> 같은 원칙이 [SOUL.md §6](SOUL.md)(사상적 뿌리 비노출)에도 적용된다.

## 세계 설정 (world/)

| 문서 | 내용 | 상태 |
|------|------|:---:|
| [world/history.md](world/history.md) | 역사 연표 | ✅ |
| [world/factions.md](world/factions.md) | 세력 설계 | ✅ |
| [world/cities.md](world/cities.md) | 도시 설정 | ✅ |
| [world/architecture.md](world/architecture.md) | 건축·영기공학 | ✅ |
| [world/cultivation.md](world/cultivation.md) | 수련 체계 (현실무공 4단계 프레임과 연결) | ✅ |
| [world/level-certification.md](world/level-certification.md) | 레벨·인증 체계 | ✅ |
| [world/lv45-tech.md](world/lv45-tech.md) | Lv4~5 기술 설정 | ✅ |
| [world/combat.md](world/combat.md) | 전투·무기 체계 | ✅ |
| [world/labor.md](world/labor.md) | 노동·기술 | ✅ |
| [world/ecology.md](world/ecology.md) | 생물 분류·마수·침식종 | ✅ |
| [world/scenario-narrative.md](world/scenario-narrative.md) | 시나리오·서사 구조 | ✅ |
| [world/law.md](world/law.md) | 사법·형벌 체계 — 율형원 (v1.0) | ✅ |
| [world/language.md](world/language.md) | 언어·문자 — 결자와 통어 (v1.0) | ✅ |
| [world/currency.md](world/currency.md) | 화폐 경제 — 영전 체계 (v1.0) | ✅ |
| [world/education.md](world/education.md) | 교육 체계 — 각성당과 조합 도제 (v1.0) | ✅ |

## 경제 시스템 (world/economy/)

| 문서 | 내용 | 상태 |
|------|------|:---:|
| [world/economy/part1_wages.md](world/economy/part1_wages.md) | 1부: 임금·소득 체계 (v1.2, 전면 확정) | ✅ |
| [world/economy/part2_cycles.md](world/economy/part2_cycles.md) | 2부: 경기순환 메커니즘 (v1.1) | ✅ |
| [world/economy/part3_mobility.md](world/economy/part3_mobility.md) | 3부: 레벨×계급 전이행렬 (v1.1) | ✅ |
| [world/economy/part4_logistics.md](world/economy/part4_logistics.md) | 4부: 물류순환망 — 축령정 물류 (v1.1) | ✅ |
| [world/economy/synthesis.md](world/economy/synthesis.md) | 종합편: 단일 캐릭터 생애 시뮬레이션 — 하란의 42년 (v1.1) | ✅ |

## 인물 (characters/)

| 문서 | 내용 | 상태 |
|------|------|:---:|
| [characters/archetypes.md](characters/archetypes.md) | 캐릭터 아키타입 | ✅ |

## 정체성

| 문서 | 내용 | 상태 |
|------|------|:---:|
| [SOUL.md](SOUL.md) | IP 정체성·톤·주제·사상적 뿌리(비노출 설계 원리) | ✅ |

## 승인된 공통 결정 (world/decisions/)

| 문서 | 내용 | 상태 |
|------|------|:---:|
| [CANON-001 · 영기 총량 회계](world/decisions/CANON-001_SPIRIT_ENERGY_ACCOUNTING.md) | 고정 경계 보존, 가용·관측량, 경계 확장·수축 및 경계간 교환의 미규명 상태 | ✅ |
| [CANON-002 · 영문 설비 접근과 경지](world/decisions/CANON-002_SPIRIT_PATTERN_ACCESS.md) | 일상 보호 사용과 기술 행위의 구분, 설비 등급별 최저 경지 | ✅ |

> 공통 결정 문서는 해당 주제의 기존 종합 문서를 명확화한다. 표현이 충돌할 경우
> 번호가 붙은 승인 결정이 우선하며, 각 결정의 "기존 문서와의 관계"를 함께 따른다.

---

### 미이관 문서

없음 — 2026-07-07 기준 캐논 이관 완료. (미작성 캐논: Lv.5 이상 상승 설계 — SOUL.md §6 참조)

### 이관 이력 (원본 → 캐논 파일명 매핑)

**1차 (2026-07-07, worldbuilding-innyeok 스킬 references/):**
`cultivation-path.md`→`cultivation.md`, `ecosystem-classification.md`→`ecology.md`, `history-chronology.md`→`history.md`, `labor-tech.md`→`labor.md`, `innyeok-v4.md`→`overview.md`, `character-archetypes.md`→`characters/archetypes.md`. 나머지는 원본 파일명 유지.

**2차 (2026-07-07, 클라우드 프로젝트 내보내기 `Downloads/innyeok_project_docs/`):**
사법·형벌 v1.0→`law.md`, 언어·문자 v1.0→`language.md`, 화폐경제 v1.0→`currency.md`, 교육체계 v1.0→`education.md`, 경제통합분석 1~4부→`economy/part1~4_*.md`, 종합편 생애시뮬레이션 v1.1→`economy/synthesis.md`.
내보내기 파일 23개 중 14개는 1차 이관분과 동일 내용(표본 대조 확인)이라 재이관하지 않음. 경제 3부 v1.0은 v1.1로 대체되어 제외.
원본 파일들은 .docx 확장자지만 실제로는 UTF-8 마크다운 텍스트임.
